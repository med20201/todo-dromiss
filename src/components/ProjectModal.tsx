...
import { supabase } from '../lib/supabaseClient' // vérifie le chemin !
...
const ProjectModal: React.FC<ProjectModalProps> = ({ isOpen, onClose, project, onProjectSaved }) => {
  ...
  // État local avec camelCase pour usage React
  const [formData, setFormData] = useState({
    ...
    start_date: '',
    end_date: '',
    teamMembers: [] as string[],  // IDs utilisateurs sélectionnés
  })

  const [teamMembersList, setTeamMembersList] = useState<any[]>([])

  // Chargement des membres de l'équipe et remplissage du formulaire si édition
  useEffect(() => {
    if (isOpen) {
      fetchTeamMembers()
      if (project) {
        setFormData({
          ...
          teamMembers: project.teammembers || [], // note la clé 'teammembers' en minuscules venant de Supabase
        })
      } else {
        setFormData({
          name: '',
  ...
  // Soumission formulaire : insert ou update Supabase
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const projectData = {
        ...
        teammembers: formData.teamMembers, // clé minuscule conforme à la table
        created_by: user.id,
        created_at: project ? undefined : new Date().toISOString(), // uniquement à la création
      }
      ...
      if (project) {
        ...
        const { error } = await supabase
          .from('projects')
          .insert([{ ...projectData, id: Date.now().toString() }])
        if (error) throw error
      }
