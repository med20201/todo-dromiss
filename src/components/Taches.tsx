  ...
  import { Plus, Filter, Search, Calendar, User , Users , AlertCircle, CheckCircle, Clock, Edit, Trash2 } from 'lucide-react'
  ...
  filteredTasks.map(task => {
    // تأكد من أن assigned_to مصفوفة من string بعد التحقق من النوع
    ...
    try {
      ...
      // تحويل كل المعرفات إلى string للتوافق مع member.id
      assignedIds = assignedIds.map(String)
    } catch {
      assignedIds = []
    }

    const assignedUsers = teamMembers.filter(member => assignedIds.includes(String(member.id)))
