...
const Projets: React.FC = () => {
  ...
  // دالة حذف مشروع عبر API مع تأكيد من المستخدم
  const handleDeleteProject = async (id: string, name: string) => {
    if (!window.confirm(`Voulez-vous vraiment supprimer le projet "${name}" ?`)) return;

    try {
      // هنا يجب تعديل الرابط حسب API الحقيقية الخاصة بك
      const response = await fetch(`http://localhost:3001/projects/${id}`, {
        method: 'DELETE',
      })
      if (!response.ok) throw new Error('Failed to delete project')
      // بعد الحذف نعيد جلب البيانات
      fetchData()
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {projects.map(project => {
            ...
            return (
              <div
                key={project.id}
                className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden cursor-pointer relative"
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-2 right-2 text-red-600 hover:text-red-800 text-sm font-bold px-2 py-1 rounded bg-red-100"
